...
from particle_life import particles, hsv
from particle_life.resize_array import resize_array
...
class Universe:
    ...
    def re_seed(self, attractMean, attractStd, minRLower, minRUpper, maxRLower, maxRUpper, friction, flatForce):
    ...
    def set_random_types(self):
        ...
        for i in range(self.types.size()):
            self.types.set_color(i, hsv.from_HSV(i / self.types.size(), 1, (i % 2) * 0.5 + 0.5))
            ...
            for j in range(self.types.size()):
                ...
                self.types.set_maxR(
                    i,
                    j,
                    max(randMaxR(self.randGen), self.types.get_minR(i, j))
                )
    ...
    def draw(self, ctx, opacity):
        ...
        for i in range(len(self.particles)):
            ...
            pygame.draw.circle(ctx, pygame.Color(col['r'], col['g'], col['b'], col['a']),(x, y), circleRadius, 0)
